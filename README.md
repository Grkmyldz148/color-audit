...
1. **The smallest step in the audit — right where feedback matters most.** Material's blue 400 (`#42a5f5`) and blue 500 (`#2196f3`) — your button and its hover state — are the audit's smallest chromatic step (distance 0.026, asserted at build time; 6× smaller than the same ramp's 50→100 jump). Side by side you can spot the seam — but hover states change *in place*, not side by side, and an in-place change this small is far easier to miss. Small, not invisible.
...
4. **Tailwind's most-used pair is its closest pair.** Blue 500→600 (the classic button + hover) is the smallest jump in the ramp — 4.9× smaller than 200→300. The seam is visible side by side; the point is that a hover happens in place, where the ramp's smallest step is the easiest to miss.
5. **One gray scale. Except it's two.** Primer's 14-step neutral is seven whisper-quiet light grays, a cliff (7→8 is **19×** the 4→5 jump), then a normal dark ramp. The 4→5 boundary (d = 0.011, 2.4× closer than Material's blue pair) is the site's genuine "spot the boundary" moment — most people will need a second look.
6. **Material red slows sharply halfway down.** Rendered as a grayscale "lightness skeleton," the four steps from 500 to 900 cover less than half the lightness distance of the five from 50 to 500 (L-CV 65.5%, the most uneven chromatic darkness ladder measured).
7. **Credit: Tailwind's blue never stops being blue.** Worst-case hue drift 9.6° — visible when the equal-lightness proof swatches face each other, but roughly a third of Primer's 27.6°. The v4 perceptual redesign pays off.
